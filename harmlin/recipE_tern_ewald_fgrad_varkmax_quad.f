...
c added FG. CHECK.....
                     egrad=ckcnoqall(i)*ckcs+cksnoqall(i)*ckss
...
c
c quadrupole-charge energy and electric field gradient:
c
                     qeng=arll*quadxx(i)+
     x                    armm*quadyy(i)+
     x                    arnn*quadzz(i)+
     x                    2.0d0*(arlm*quadxy(i)+
     x                           arln*quadxz(i)+armn*quadyz(i))
c
c factor of onethird from Buckingham.
c
                     qquadeng=qquadeng-onethird*egrad*qeng
...
c
      ...
      qquadeng=qquadeng*2.0d0*fourpicell
