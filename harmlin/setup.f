...
c****            open(801+nnn,file=filenamexx,status='new')
c****            open(802+nnn,file=filenameyy,status='new')
c****            open(803+nnn,file=filenamezz,status='new')
c****            open(804+nnn,file=filenamexy,status='new')
c****            open(805+nnn,file=filenamexz,status='new')
c****            open(806+nnn,file=filenameyz,status='new')

c****            open(921+nnn,file=filenamesrxx,status='new')
c****            open(922+nnn,file=filenamesryy,status='new')
c****            open(923+nnn,file=filenamesrzz,status='new')
c****            open(924+nnn,file=filenamesrxy,status='new')
c****            open(925+nnn,file=filenamesrxz,status='new')
c****            open(926+nnn,file=filenamesryz,status='new')
...
c
      ...
      open(86,file='engpart.out',status='new')
...
c
     ...
     x              xk1(i)=1.0d0/(2.0d0*alppolar(i))
         if (quadpimlog) then
            ...
            xk2(i)=-4.0d0*xk1(i)*xk1(i)*xk3(i)*Bpolar(i)
            xk4(i)=-xk2(i)*Bpolar(i)/(6.0d0*alppolar(i)*alppolar(i))
         endif
  550 continue
  ...
  440    continue
      ...
      if (quadpimlog) then
      	 ...
      	 do 445 i=1,nspec
            ...
            quadtkb(i)=boltz*quadtemp(i)
            quadrlxsq(i)=quadrelax(i)*quadrelax(i)
            quadrlxsqrec(i)=1.0d0/quadrlxsq(i)
            tfluxquad(i)=0.0d0
            quadmassrec(i)=1.0d0/quadmass(i)
            gquad(i)=5.0d0*dble(nsp(i))*quadtkb(i)
            gquadrec(i)=1.0d0/gquad(i)
            gquadtim(i)=gquad(i)*dtime/quadtemp(i)
            gquadkin(i)=1.0d0/(5.0d0*dble(nsp(i))*boltz)
 ...
 445	 continue
      ...
      if ((cimlog).or.(daimlog).or.(quaimlog)) then
         do 991 i=1,num
            ...
            deltavel(i)=0.0d0
  991    continue
         deltatkb=boltz*deltatemp
         deltarlxsq=deltarelax*deltarelax
         deltarlxsqrec=1.0d0/deltarlxsq
         selfmassrec=1.0d0/selfmass
c assumes there is only one compressible species and
c that it is the first species read in (i.e nsp(1)).
         gdelta=dble(nsp(1))*deltatkb
         gdeltarec=1.0d0/gdelta
         gdeltatim=gdelta*dtime/deltatemp
         gdeltakin=1.0d0/(dble(nsp(1))*boltz)
      endif

      if ((daimlog).or.(quaimlog)) then
         epstkb=boltz*epstemp
         epsrlxsq=epsrelax*epsrelax
         epsrlxsqrec=1.0d0/epsrlxsq
         epsmassrec=1.0d0/epsmass
         geps=3.0d0*dble(nsp(1))*epstkb
         gepsrec=1.0d0/geps
         gepstim=geps*dtime/epstemp
         gepskin=1.0d0/(3.0d0*dble(nsp(1))*boltz)
      endif

      if (quaimlog) then
         quaimtkb=boltz*quaimtemp
         quaimrlxsq=quaimrelax*quaimrelax
         quaimrlxsqrec=1.0d0/quaimrlxsq
         quaimmassrec=1.0d0/quaimmass
         gquaim=5.0d0*dble(nsp(1))*quaimtkb
         gquaimrec=1.0d0/gquaim
         gquaimtim=gquaim*dtime/quaimtemp
         gquaimkin=1.0d0/(5.0d0*dble(nsp(1))*boltz)
      endif
c
...
c=====================================================================
c Set up the light scattering variables.
      ...
      do 91 i=1,nsymmax
         do 95 j=1,ncontribmax
            cohav(i,j)=0.0d0
   95    continue
         do 92 j=1,nindexmax
               do 93 k=0,ncorrtimemax
                  lnorm(i,j,k)=0
                  scf(i,j,k)=0.0d0
   93          continue
   92    continue
...
c=====================================================================
c Multiply the inputed light scattering preexponents by the
c exponential of the sum of the radii to allow direct use in
c the energy calc routines.
...
c=====================================================================
c Set up array to allow the DID terms to be calculated.
      do 765 ii=1,15
         do 765 jj=1,ncfmatmax2
            do 765 kk=0,ncorrtimemax
...
c
  ...
  655 continue
      if (fixedzlog) then
         do 656 i=1,nionsfixedz
            fixedzarray(nfixedz(i))=.true.
  656    continue
      endif

      do 432 i=1,num
        write(76,*)fixedarray(i),fixedzarray(i)
  432 continue

               write(6,*)'sdfdsfsdfsdfdsfsdfdsf',sw2potlog,sw3potlog
      ...
      if ((sw2potlog).or.(sw3potlog)) then
         ...
         do 700 i=1,nspec
            ...
            do 710 j=1,nspec
               ...
               swepsilon(i,j)=swepsilon(i,j)*rkb

               swsigma(i,j)=swsigma(i,j)/0.529177d0

               write(6,*)i,j,swsigma(i,j)
  710       continue
