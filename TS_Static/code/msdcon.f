      ...
      parameter (nmsdcorrmax=1000)
      parameter (nummax=256)
      parameter (nspmax=3)
     ...
     &                normtot(0:nmsdcorrmax )
      ...
      common/timest/dtime
      common/ntype/ncation1,ncation2,nanion
...
c     open(11,file='disp.out',status='old',form='unformatted')
      open(11,file='disp.out',status='old')
...
10    continue
      ...
      do 12 ns=1,nspecies
          ...
          open (102+ns,file=filename)
          do 11 ns2=ns,nspecies
              filename='msdcollect'//char(ns+48)//char(ns2+48)//'.dat'
              open (120+(ns*nspecies)+ns2,file=filename)
11        continue
12    continue
      open (100,file='work.dat')
      open (101,file='nernst.dat')
...
50    continue
      ...
      if (restart) then
          ...
          open(12,file=rstfile,status='OLD',form='unformatted')
...
c   Writing out values for restart file
      ...
      open (14,file=rstfile,form='unformatted')
...
150   continue
      ...
      do 200 i=0,nmsdlength-1
          time=dble(i)*dble(nmsdcalltime)*dtime*2.418d-17

          do 160 ip=1,nspecies
              write (102+ip,*) time,msd(i,ip)
160       continue
          ...
          do 180 ip1=1,nspecies
              ...
              do 170 ip2=ip1,nspecies
                  write(120+(ip1*nspecies)+ip2,*) time,
     &                           msdcoll(i,ip1,ip2)
                  msdcoll(i,ip1,ip2)=msdcoll(i,ip1,ip2)*z(ip1)*z(ip2)*
